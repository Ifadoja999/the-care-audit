...
export default function ForFacilitiesPage() {
  ...
  // Show tier cards when no facility is selected (preview) or when selected and has inspection data
  const showTierCards = !selected || hasInspectionData;
  // Show premium tiers (FV + VP) when previewing (no selection) or selected with <=3 violations
  ...
  return (
    <div className="flex min-h-screen flex-col bg-warm-50">
      ...
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-8">
        ...
        {/* Pricing tiers — always visible above the fold (hide only when facility selected with no inspection data) */}
        {showTierCards && <div className="mt-10">
          {showPremiumTiers ? (
            <div className="mx-auto grid max-w-3xl gap-6 sm:grid-cols-2">
              {/* Featured Verified */}
              <div className={`rounded-2xl border-2 border-amber-300 bg-white p-6 shadow-md transition-opacity ${!selected ? 'opacity-70' : ''}`}>
                ...
                {selected ? (
                  ...
                  <div className="mt-6 w-full rounded-xl bg-amber-100 py-3 text-center text-sm font-medium text-amber-700">
                    Find your facility below to subscribe
                  </div>
                )}
              </div>

              {/* Verified Profile */}
              <div className={`rounded-2xl border border-gray-200 bg-white p-6 shadow-sm transition-opacity ${!selected ? 'opacity-70' : ''}`}>
                <div className="mb-4 flex items-center gap-2">
                ...
                {selected ? (
                  ...
                  <div className="mt-6 w-full rounded-xl bg-gray-100 py-3 text-center text-sm font-medium text-gray-600">
                    Find your facility below to subscribe
                  </div>
                )}
              </div>
            ...
            <div className={`mx-auto max-w-md transition-opacity ${!selected ? 'opacity-70' : ''}`}>
        ...
        {/* Search — positioned as "Step 1" */}
        <div className="mx-auto mt-12 max-w-xl">
          <h2 className="mb-3 text-center text-xl font-semibold text-gray-900">
            {selected ? 'Your Facility' : 'Find Your Facility to Get Started'}
          </h2>

          {autoLoading ? (
            <div className="rounded-xl border border-gray-200 bg-white px-4 py-5 text-center text-sm text-gray-500 shadow-sm">
              Loading your facility&hellip;
            </div>
          ) : (
        ...
        {selected && (
          <div className="mx-auto mt-4 max-w-xl rounded-xl border border-gray-200 bg-white p-4 text-center shadow-sm">
