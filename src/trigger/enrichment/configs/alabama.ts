/**
 * Alabama (AL) enrichment config — ADPH Deficiencies Portal.
 *
 * Portal: https://dph1.adph.state.al.us/Deficiencies/
 * PDFs:   https://dph1.adph.state.al.us/DeficienciesReports/{filename}.pdf
 *
 * Architecture:
 * - Playwright required for ASP.NET WebForms portal navigation (search + AJAX panels)
 * - PDF downloads are direct HTTP GET (no session, no auth, no cookies)
 * - 100% text-extractable CMS-2567 format PDFs — no Vision OCR needed
 * - First 3 + Last 2 page strategy (max 5 pages per document)
 *
 * ⚠️ CRITICAL: ASP.NET WebForms uses UpdatePanels for AJAX. After every user
 * action that triggers a postback (dropdown change, button click, link click),
 * we MUST wait for `Sys.WebForms.PageRequestManager` to finish its async postback.
 * Static timeouts are NOT reliable — the PageRequestManager check is the gold standard.
 *
 * ID Mapping:
 * - Supabase license_number: "AL-D4933" (ALF) or "AL-P4107" (SCALF)
 * - Portal FacID: "D4933" or "P4107" (strip "AL-" prefix)
 * - D prefix = ALF (dropdown value "D"), P prefix = SCALF (dropdown value "P")
 *
 * Navigation flow per facility:
 * 1. Select facility type dropdown (D or P) — dispatch change event for __doPostBack
 * 2. Wait for ASP.NET AJAX postback to complete (PageRequestManager check)
 * 3. Set search type to "Contains", enter facility name, click Search
 * 4. Wait for AJAX postback, then find correct row by FacID in results grid
 * 5. Click "Select" link, wait for AJAX detail panel
 * 6. Click most recent survey date to expand file list
 * 7. Extract SOD PDF URL from the file link
 * 8. Download PDF via direct HTTP (no session needed)
 * 9. Extract text via pdftotext (first 3 + last 2 pages)
 * 10. AI analysis → Supabase write
 */

import { logger } from "@trigger.dev/sdk";
import * as fs from "fs";
import * as path from "path";
import type { StateEnrichmentConfig, DocumentInfo, FacilityToEnrich } from "../types";
import { downloadPdf, extractPdfContent, cleanupTempFiles } from "../lib/pdf-extractor";

const PORTAL_URL = "https://dph1.adph.state.al.us/Deficiencies/";
const TEMP_DIR = "/tmp/tca-enrichment";

// Known FacIDs that are NOT in the deficiencies portal (newly licensed)
const KNOWN_MISSING_IDS = [
  "D5281", "D5280", "D5278", "D5268", "D5266",
  "D5265", "D5262", "D5249", "D5248", "D5250",
];

/**
 * Wait for ASP.NET AJAX PageRequestManager to finish its async postback.
 * This is the reliable way to wait for UpdatePanel content to load.
 *
 * Falls back to networkidle if PageRequestManager doesn't exist.
 */
async function waitForAjaxComplete(page: any, label: string, timeoutMs: number = 15000): Promise<void> {
  try {
    await page.waitForFunction(
      () => {
        const prm = (window as any).Sys?.WebForms?.PageRequestManager?.getInstance?.();
        if (prm) {
          return !prm.get_isInAsyncPostBack();
        }
        return true;
      },
      { timeout: timeoutMs },
    );
    logger.info(`  [AJAX] ${label} — postback complete`);
  } catch {
    logger.warn(`  [AJAX] ${label} — waitForFunction timed out after ${timeoutMs}ms, falling back to networkidle`);
    await page.waitForLoadState("networkidle", { timeout: 5000 }).catch(() => {});
  }
  // Small buffer for DOM updates after AJAX resolves
  await page.waitForTimeout(500);
}

/**
 * Log diagnostic info about the page state: key element presence and values.
 */
async function logPageDiagnostics(page: any, label: string): Promise<void> {
  const diagnostics = await page.evaluate(() => {
    const dropdown = document.querySelector("#MainContent_DropDownListFacTypes") as HTMLSelectElement | null;
    const searchType = document.querySelector("#MainContent_DropDownListSearchType") as HTMLSelectElement | null;
    const searchBox = document.querySelector("#MainContent_TextBoxSearch") as HTMLInputElement | null;
    const searchBtn = document.querySelector("#MainContent_ButtonSearch") as HTMLElement | null;
    const grid = document.querySelector("#MainContent_GridView1") as HTMLElement | null;

    // Find all submit/button elements for selector discovery
    const allInputs = Array.from(document.querySelectorAll("input[type='submit'], input[type='button'], button"))
      .map((el) => ({
        tag: el.tagName,
        id: el.getAttribute("id") || "",
        name: el.getAttribute("name") || "",
        value: (el as HTMLInputElement).value || el.textContent?.trim() || "",
        type: el.getAttribute("type") || "",
      }))
      .filter((b) => b.id || b.value);

    return {
      title: document.title,
      dropdownExists: !!dropdown,
      dropdownValue: dropdown?.value ?? null,
      dropdownSelectedText: dropdown?.options?.[dropdown.selectedIndex]?.text ?? null,
      dropdownOptions: dropdown
        ? Array.from(dropdown.options).map((o) => ({ value: o.value, text: o.text }))
        : [],
      searchTypeExists: !!searchType,
      searchTypeValue: searchType?.value ?? null,
      searchBoxExists: !!searchBox,
      searchBoxValue: searchBox?.value ?? null,
      searchBtnExists: !!searchBtn,
      searchBtnTag: searchBtn?.tagName ?? null,
      searchBtnType: searchBtn?.getAttribute("type") ?? null,
      gridExists: !!grid,
      gridRowCount: grid ? grid.querySelectorAll("tr").length : 0,
      allButtons: allInputs,
    };
  });

  logger.info(`  [DIAG:${label}] ${JSON.stringify(diagnostics)}`);
}

/**
 * Log the HTML content of the results area (truncated) for debugging.
 */
async function logResultsArea(page: any): Promise<void> {
  const info = await page.evaluate(() => {
    const grid = document.querySelector("#MainContent_GridView1");
    if (grid) {
      const rows = grid.querySelectorAll("tr");
      const rowTexts: string[] = [];
      rows.forEach((row: Element, i: number) => {
        if (i < 10) {
          const cells = Array.from(row.querySelectorAll("td, th")).map((c) => c.textContent?.trim() || "");
          rowTexts.push(`Row ${i}: [${cells.join(" | ")}]`);
        }
      });
      return `GRID FOUND (${rows.length} rows):\n${rowTexts.join("\n")}`;
    }

    const mainContent = document.querySelector("#MainContent, [id*='MainContent'], form");
    if (mainContent) {
      const text = mainContent.textContent?.replace(/\s+/g, " ").trim().substring(0, 1500) || "";
      return `NO GRID FOUND. MainContent text: ${text}`;
    }

    return "NO MainContent or GRID found on page";
  });

  logger.info(`  [RESULTS] ${info}`);
}

/**
 * Take a screenshot and save to /tmp for debugging.
 */
async function takeScreenshot(page: any, portalId: string, step: string): Promise<void> {
  try {
    const buffer = await page.screenshot({ fullPage: true });
    const filename = `/tmp/al-${portalId}-${step}.png`;
    fs.writeFileSync(filename, buffer);
    logger.info(`  [SCREENSHOT] ${filename} (${buffer.length} bytes)`);
  } catch (err: any) {
    logger.warn(`  [SCREENSHOT] Failed for ${step}: ${err.message}`);
  }
}

/**
 * Extract a clean search term from the facility name.
 * Uses first 2-3 significant words to avoid over-specific searches.
 */
function getSearchTerm(facilityName: string): string {
  const cleaned = facilityName
    .replace(/\b(LLC|Inc|II|III|IV|ALF|SCALF|of\s+\w+)\b/gi, "")
    .replace(/[^\w\s]/g, "")
    .trim();

  const words = cleaned.split(/\s+/).filter((w) => w.length > 1);
  const termWords = words.slice(0, Math.min(3, words.length));
  return termWords.join(" ");
}

/**
 * Parse survey date strings in M/D/YYYY format.
 * e.g., "9/2/2021" → "2021-09-02"
 */
function parseSurveyDate(dateStr: string): string | null {
  const match = dateStr.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
  if (!match) return null;
  const month = match[1].padStart(2, "0");
  const day = match[2].padStart(2, "0");
  return `${match[3]}-${month}-${day}`;
}

/**
 * Navigate the portal, search for a facility, select it, find the survey PDF,
 * download it, and extract text. Returns extracted text + survey date, or null
 * if the facility/document wasn't found.
 */
async function extractFacilityFromPortal(
  facility: FacilityToEnrich,
  portalId: string,
  browser: any,
): Promise<{ text: string; surveyDate: string | null; pageCount: number } | null> {
  const facilityType = portalId.charAt(0); // "D" or "P"
  const context = await browser.newContext({ ignoreHTTPSErrors: true });
  const page = await context.newPage();

  try {
    // ─── Step 1: Navigate to portal ──────────────────────────────
    logger.info(`  [STEP 1] Navigating to portal for ${portalId}...`);
    await page.goto(PORTAL_URL, { waitUntil: "networkidle", timeout: 30000 });
    await page.waitForTimeout(1500);

    await logPageDiagnostics(page, "after-load");

    // ─── Step 2: Select facility type dropdown ───────────────────
    logger.info(`  [STEP 2] Selecting facility type: "${facilityType}"...`);

    await page.selectOption("#MainContent_DropDownListFacTypes", facilityType);

    // CRITICAL: Dispatch change event to trigger ASP.NET __doPostBack.
    // Playwright's selectOption fires input+change events but ASP.NET WebForms
    // may bind handlers via attachEvent/onchange attributes that need explicit dispatch.
    await page.dispatchEvent("#MainContent_DropDownListFacTypes", "change");

    // Wait for any AJAX postback triggered by dropdown change
    await waitForAjaxComplete(page, "dropdown-change");

    // Verify the dropdown actually changed
    const dropdownCheck = await page.evaluate(() => {
      const el = document.querySelector("#MainContent_DropDownListFacTypes") as HTMLSelectElement | null;
      return el ? { value: el.value, text: el.options[el.selectedIndex]?.text } : null;
    });
    logger.info(`  [STEP 2] Dropdown value: ${JSON.stringify(dropdownCheck)}`);

    if (dropdownCheck && dropdownCheck.value !== facilityType) {
      logger.error(`  [STEP 2] Dropdown did NOT change! Expected "${facilityType}", got "${dropdownCheck.value}"`);
      // Fallback: set value directly via JS and trigger change
      await page.evaluate((type: string) => {
        const el = document.querySelector("#MainContent_DropDownListFacTypes") as HTMLSelectElement;
        if (el) {
          el.value = type;
          el.dispatchEvent(new Event("change", { bubbles: true }));
        }
      }, facilityType);
      await waitForAjaxComplete(page, "dropdown-change-retry");
    }

    // ─── Step 3: Set search type and enter search term ───────────
    const searchTerm = getSearchTerm(facility.facility_name);
    logger.info(`  [STEP 3] Searching: "${searchTerm}" (type: ${facilityType}, portalId: ${portalId})`);

    await page.selectOption("#MainContent_DropDownListSearchType", "Contains");
    await page.waitForTimeout(300);

    await page.fill("#MainContent_TextBoxSearch", "");
    await page.waitForTimeout(200);
    await page.fill("#MainContent_TextBoxSearch", searchTerm);

    // Verify search field
    const searchFieldValue = await page.evaluate(() => {
      const el = document.querySelector("#MainContent_TextBoxSearch") as HTMLInputElement | null;
      return el?.value ?? null;
    });
    logger.info(`  [STEP 3] Search field value: "${searchFieldValue}"`);

    // ─── Step 4: Click Search button ─────────────────────────────
    logger.info(`  [STEP 4] Clicking search button...`);

    const btnExists = await page.locator("#MainContent_ButtonSearch").count();
    if (btnExists === 0) {
      logger.error("  [STEP 4] #MainContent_ButtonSearch NOT FOUND!");
      await logPageDiagnostics(page, "no-button");
      await takeScreenshot(page, portalId, "no-button");

      // Try alternate selectors
      const altSelectors = [
        "input[value='Search']",
        "input[value='search']",
        "#MainContent input[type='submit']",
        "[id*='ButtonSearch']",
        "[id*='btnSearch']",
      ];

      let foundAlt = false;
      for (const sel of altSelectors) {
        const count = await page.locator(sel).count();
        if (count > 0) {
          logger.info(`  [STEP 4] Found alternate button: "${sel}"`);
          await page.click(sel);
          foundAlt = true;
          break;
        }
      }

      if (!foundAlt) {
        logger.error("  [STEP 4] NO search button found with any selector!");
        await takeScreenshot(page, portalId, "no-button-at-all");
        return null;
      }
    } else {
      await page.click("#MainContent_ButtonSearch");
    }

    // Wait for AJAX postback after search
    await waitForAjaxComplete(page, "search-click", 20000);

    // Debug: screenshot + results HTML
    await takeScreenshot(page, portalId, "after-search");
    await logResultsArea(page);

    // ─── Step 5: Find correct facility in results grid ───────────
    logger.info(`  [STEP 5] Looking for FacID "${portalId}" in results grid...`);
    let matchFound = await findAndSelectFacility(page, portalId);

    if (!matchFound) {
      // Retry with shorter search term (first word only)
      const shortTerm = searchTerm.split(" ")[0];
      if (shortTerm !== searchTerm && shortTerm.length >= 3) {
        logger.info(`  [STEP 5] No match — retrying with: "${shortTerm}"`);

        await page.goto(PORTAL_URL, { waitUntil: "networkidle", timeout: 30000 });
        await page.waitForTimeout(1500);

        await page.selectOption("#MainContent_DropDownListFacTypes", facilityType);
        await page.dispatchEvent("#MainContent_DropDownListFacTypes", "change");
        await waitForAjaxComplete(page, "retry-dropdown");

        await page.selectOption("#MainContent_DropDownListSearchType", "Contains");
        await page.fill("#MainContent_TextBoxSearch", shortTerm);
        await page.click("#MainContent_ButtonSearch");
        await waitForAjaxComplete(page, "retry-search", 20000);

        await takeScreenshot(page, portalId, "after-retry-search");
        await logResultsArea(page);

        matchFound = await findAndSelectFacility(page, portalId);
      }

      if (!matchFound) {
        logger.warn(`  [STEP 5] Facility ${portalId} NOT FOUND in portal after all attempts`);
        return null;
      }
    }

    // ─── Step 6: Read survey list from detail panel ──────────────
    logger.info(`  [STEP 6] Facility selected — reading survey list...`);
    await waitForAjaxComplete(page, "select-facility");
    await takeScreenshot(page, portalId, "after-select");

    const surveyData = await extractSurveyAndPdfUrl(page, portalId);

    if (!surveyData) {
      logger.info(`  [STEP 6] No surveys found for ${portalId}`);
      return null;
    }

    logger.info(`  [STEP 6] Survey date: ${surveyData.surveyDate}, PDF: ${surveyData.pdfUrl ? "yes" : "no"}`);

    if (!surveyData.pdfUrl) {
      logger.warn(`  [STEP 6] Survey found but no PDF URL for ${portalId}`);
      return null;
    }

    // ─── Step 7: Download PDF and extract text ───────────────────
    logger.info(`  [STEP 7] Downloading PDF: ${surveyData.pdfUrl}`);

    if (!fs.existsSync(TEMP_DIR)) {
      fs.mkdirSync(TEMP_DIR, { recursive: true });
    }

    const pdfPath = path.join(TEMP_DIR, `al-${portalId}-${Date.now()}.pdf`);

    try {
      await downloadPdf(surveyData.pdfUrl, pdfPath);

      const fileSizeBytes = fs.statSync(pdfPath).size;
      logger.info(`  [STEP 7] PDF downloaded: ${(fileSizeBytes / 1024).toFixed(0)}KB`);

      const result = await extractPdfContent(pdfPath, {
        pageStrategy: "first3last2",
      });

      logger.info(`  [STEP 7] Extracted ${result.text.length} chars (${result.method} method, ${result.pagesExtracted.length} pages)`);

      return {
        text: result.text,
        surveyDate: surveyData.surveyDate,
        pageCount: result.pageCount,
      };
    } finally {
      try {
        if (fs.existsSync(pdfPath)) fs.unlinkSync(pdfPath);
      } catch { /* ignore */ }
      cleanupTempFiles();
    }
  } finally {
    // Always close the context to free Playwright resources
    await context.close().catch(() => {});
  }
}

/**
 * Find the correct facility row in the results grid by FacID and click "Select".
 */
async function findAndSelectFacility(page: any, targetFacId: string): Promise<boolean> {
  const gridExists = await page.locator("#MainContent_GridView1").count();
  if (gridExists === 0) {
    logger.warn(`  [GRID] #MainContent_GridView1 not found`);

    // Try alternate grid selectors
    for (const sel of ["[id*='GridView']", "table.GridView", "#MainContent table"]) {
      const count = await page.locator(sel).count();
      if (count > 0) {
        logger.info(`  [GRID] Found alternate: "${sel}" (count: ${count})`);
      }
    }
    return false;
  }

  const rows = await page.locator("#MainContent_GridView1 tr").all();
  logger.info(`  [GRID] Found ${rows.length} rows in results grid`);

  // Skip header row (index 0)
  for (let rowIdx = 1; rowIdx < rows.length; rowIdx++) {
    const cells = await rows[rowIdx].locator("td").all();
    if (cells.length < 3) continue;

    // FacID is in the second cell (index 1)
    const facIdText = (await cells[1].textContent())?.trim();

    // Log first few rows for debugging
    if (rowIdx <= 5) {
      const cellTexts: string[] = [];
      for (let c = 0; c < Math.min(cells.length, 5); c++) {
        cellTexts.push((await cells[c].textContent())?.trim() || "");
      }
      logger.info(`  [GRID] Row ${rowIdx}: [${cellTexts.join(" | ")}]`);
    }

    if (facIdText === targetFacId) {
      logger.info(`  [GRID] Found match at row ${rowIdx}: FacID="${facIdText}"`);
      const selectLink = cells[0].locator("a");
      const linkCount = await selectLink.count();
      if (linkCount > 0) {
        await selectLink.click();
        await waitForAjaxComplete(page, "select-click");
        return true;
      } else {
        logger.warn(`  [GRID] No "Select" link in first cell of matching row`);
      }
    }
  }

  logger.warn(`  [GRID] Target FacID "${targetFacId}" not found in ${rows.length - 1} data rows`);
  return false;
}

/**
 * After selecting a facility, extract the most recent survey date and SOD PDF URL
 * from the AJAX detail panel.
 */
async function extractSurveyAndPdfUrl(
  page: any,
  portalId: string,
): Promise<{ surveyDate: string | null; pdfUrl: string | null } | null> {
  // Look for survey date links
  const detailPanel = page.locator("#MainContent_FormView1, #MainContent_DetailsView1, .aspNetHidden + div, #MainContent_Panel1");
  const panelExists = await detailPanel.first().count();

  logger.info(`  [SURVEY] Detail panel exists: ${panelExists > 0}`);

  // Search for date-formatted links across the page
  const allLinks = await page.locator("#MainContent a").all();
  logger.info(`  [SURVEY] Total links in #MainContent: ${allLinks.length}`);

  let surveyDateLink: { element: any; date: string } | null = null;
  let latestDate: Date | null = null;

  for (const link of allLinks) {
    const text = (await link.textContent())?.trim() || "";
    if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(text)) {
      const parts = text.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
      if (parts) {
        const date = new Date(parseInt(parts[3]), parseInt(parts[1]) - 1, parseInt(parts[2]));
        logger.info(`  [SURVEY] Found survey date: "${text}" (${date.toISOString().split("T")[0]})`);
        if (!latestDate || date > latestDate) {
          latestDate = date;
          surveyDateLink = { element: link, date: text };
        }
      }
    }
  }

  if (!surveyDateLink) {
    // No date links found — check for direct PDF links
    logger.info("  [SURVEY] No survey date links found — checking for direct PDF links");
    const directPdf = await findSodPdfLink(page);
    if (directPdf) {
      logger.info(`  [SURVEY] Found direct PDF: ${directPdf}`);
      return { surveyDate: null, pdfUrl: directPdf };
    }

    // Log what links ARE there for debugging
    const linkTexts: string[] = [];
    for (const link of allLinks.slice(0, 20)) {
      const text = (await link.textContent())?.trim() || "";
      const href = await link.getAttribute("href");
      if (text || href) linkTexts.push(`"${text}" → ${href}`);
    }
    logger.info(`  [SURVEY] Links found: ${linkTexts.join(", ")}`);
    return null;
  }

  // Click the most recent survey date to expand file list
  const surveyDate = parseSurveyDate(surveyDateLink.date);
  logger.info(`  [SURVEY] Clicking most recent: "${surveyDateLink.date}" (${surveyDate})`);
  await surveyDateLink.element.click();
  await waitForAjaxComplete(page, "survey-expand");

  await takeScreenshot(page, portalId, "after-survey-expand");

  const pdfUrl = await findSodPdfLink(page);
  return { surveyDate, pdfUrl };
}

/**
 * Search for a Statement of Deficiencies (SOD) PDF link on the page.
 * Prefers SOD over POC (Plan of Correction).
 */
async function findSodPdfLink(page: any): Promise<string | null> {
  const allLinks = await page.locator("a[href]").all();

  let sodUrl: string | null = null;
  let anyPdfUrl: string | null = null;

  for (const link of allLinks) {
    const href = await link.getAttribute("href");
    if (!href) continue;

    if (href.includes("DeficienciesReports") || href.includes("deficienciesreports")) {
      const text = (await link.textContent())?.trim().toLowerCase() || "";

      let fullUrl = href;
      if (!href.startsWith("http")) {
        fullUrl = new URL(href, "https://dph1.adph.state.al.us/").href;
      }

      logger.info(`  [PDF] Found report link: text="${text}", href="${fullUrl}"`);

      if (text.includes("sod") || text.includes("statement") || text.includes("deficien")) {
        sodUrl = fullUrl;
      }
      if (!anyPdfUrl) {
        anyPdfUrl = fullUrl;
      }
    }
  }

  return sodUrl || anyPdfUrl;
}

export const alabamaConfig: StateEnrichmentConfig = {
  stateCode: "AL",
  stateName: "Alabama",
  portalBaseUrl: PORTAL_URL,
  pdfType: "text",
  maxPagesPerDocument: 5,
  maxConcurrency: 1, // Sequential — ASP.NET session is stateful
  delayBetweenRequests: 2000,
  requiresPlaywright: true,

  extractPortalId: (licenseNumber: string) => licenseNumber.replace("AL-", ""),

  // Not used by extractAllData path, but required by interface
  selectDocument: (documents: DocumentInfo[]): DocumentInfo | null => {
    if (documents.length === 0) return null;
    return documents[0];
  },

  getReportUrl: (_portalId: string) => PORTAL_URL,

  /**
   * Full Playwright-based extraction for Alabama's ASP.NET WebForms portal.
   * Handles portal navigation, document discovery, PDF download, and text extraction.
   */
  extractAllData: async (
    facility: FacilityToEnrich,
    portalId: string,
    browser: any,
  ): Promise<{ text: string; surveyDate: string | null; pageCount: number } | null> => {
    if (!browser) {
      throw new Error("Alabama requires Playwright but no browser provided");
    }

    // Skip known missing IDs
    if (KNOWN_MISSING_IDS.includes(portalId)) {
      logger.info(`  Skipping ${portalId} — known to not be in portal (newly licensed)`);
      return null;
    }

    return extractFacilityFromPortal(facility, portalId, browser);
  },
};
