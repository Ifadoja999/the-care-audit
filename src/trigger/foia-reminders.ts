...
export const foiaFollowUpCheck = schedules.task({
  id: "foia-follow-up-check",
  cron: "0 13 * * *", // 13:00 UTC = 8:00 AM CT
  run: async (payload) => {
    // PAUSED: Only run if explicitly enabled via Trigger.dev env var
    // To re-enable: set FOIA_REMINDERS_ENABLED=true in Trigger.dev Production env vars, then redeploy
    if (process.env.FOIA_REMINDERS_ENABLED !== "true") {
      logger.info("FOIA reminders are disabled (FOIA_REMINDERS_ENABLED != true). Skipping.");
      return { reminders: 0, skipped: true };
    }

    const startedAt = new Date();
    const today = new Date().toISOString().split("T")[0];

    // Find requests with follow-up due today or overdue
    const { data: dueRequests } = await supabase
      .from("foia_requests")
      .select("*")
      .in("status", ["submitted", "acknowledged", "processing"])
    ...
    for (const request of dueRequests) {
      ...
      // Set next follow-up: 14 days from now
    ...
    // Also check for overdue responses (past expected_response_date)
...
function buildFoiaReminderEmail(request: any): string {
  const daysSinceSubmit = Math.floor(
    (Date.now() - new Date(request.submitted_date).getTime()) /
      (1000 * 60 * 60 * 24)
  );
...
function buildOverdueEmail(requests: any[]): string {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px;">
